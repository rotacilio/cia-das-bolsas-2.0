...
class CreateCategoryDialog(val activity: CategoriesActivity) : DialogFragment(), View.OnClickListener {
        ...
        when (v?.id) {
            ...
            R.id.btnSave -> {
                if (isValidData()) {
                    ...
                    categoriesRequests.createNewCategory(category, object : OnCompleteRequestListener<Category> {
    ...
    companion object {
        fun newInstance(activity: CategoriesActivity): CreateCategoryDialog = CreateCategoryDialog(activity)
    }
}